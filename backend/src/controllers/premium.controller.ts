...
export const unblurProfile = async (req: Request, res: Response) => {
    try {
        const userId = req.user?.userId;
        const { targetId } = req.body;
        if (!userId || !targetId) return res.status(400).json({ error: 'Missing target' });

        // Prevent self-unblur
        if (userId === targetId) return res.status(400).json({ error: 'Cannot unblur yourself' });
        ...
        const existingUnblur = await prisma.friendship.findFirst({
            where: {
                OR: [
...
export const superLike = async (req: Request, res: Response) => {
    try {
        const userId = req.user?.userId;
        const { targetId } = req.body;
        if (!userId || !targetId) return res.status(400).json({ error: 'Parameters missing' });

        // Prevent self super-like
        if (userId === targetId) return res.status(400).json({ error: 'Cannot super-like yourself' });
...
export const addExtraTime = async (req: Request, res: Response) => {
    try {
        const userId = req.user?.userId;
