...
export interface PlanetPosition {
    ...
    sign: string;        // Burcu
    degree: number;      // Burç içindeki derece
    retrograde?: boolean;
}
...
const ASPECT_TYPES = [
    { name: 'conjunction', angle: 0, orb: 8, nature: 'neutral' as const, symbol: '☌', tr: 'Kavuşum' },
    { name: 'sextile', angle: 60, orb: 6, nature: 'harmonious' as const, symbol: '⚹', tr: 'Sekstil' },
    { name: 'square', angle: 90, orb: 7, nature: 'challenging' as const, symbol: '□', tr: 'Kare' },
    { name: 'trine', angle: 120, orb: 8, nature: 'harmonious' as const, symbol: '△', tr: 'Trigon' },
    { name: 'opposition', angle: 180, orb: 8, nature: 'challenging' as const, symbol: '☍', tr: 'Karşıtlık' },
];
...
export function calculatePlanetaryPositions(birthDate: Date, birthTime: string): PlanetPosition[] {
    ...
    const calculators: [string, (jd: number) => number][] = [
...
export function calculateAspects(planets1: PlanetPosition[], planets2: PlanetPosition[]): Aspect[] {
...
function generateAspectInterpretation(planet1: string, planet2: string, aspectType: string, nature: string): string {
...
function calculateCategoryScores(aspects: Aspect[]): SynastryCategory[] {
    ...
    function scoreCat(relevantPairs: Set<string>): number {
...
function getScoreDescription(score: number, prefix: string): string {
    if (score >= 85) return `${prefix} olağanüstü güçlü! Kozmik bir bağ var aranızda.`;
    if (score >= 70) return `${prefix} çok iyi seviyede. Doğal bir uyum hissedeceksiniz.`;
    if (score >= 55) return `${prefix} iyi durumda. Biraz çaba ile mükemmelleşebilir.`;
    if (score >= 40) return `${prefix} gelişim alanları barındırıyor. Birbirinizden öğreneceğiniz çok şey var.`;
...
export function calculateSynastryReport(
    user1: { birthDate: Date; birthTime: string; name?: string; moonSign?: string; risingSign?: string },
    user2: { birthDate: Date; birthTime: string; name?: string; moonSign?: string; risingSign?: string }
): SynastryReport {
...
export function calculateQuickSynastryScore(
    ...
    let message: string;
    if (score >= 85) message = 'Olağanüstü kozmik bağ! Yıldızlar sizin için parlıyor.';
    else if (score >= 70) message = 'Güçlü bir uyum. Doğal bir çekim ve ahenk var.';
    else if (score >= 55) message = 'Dengeli bir potansiyel. Birbirinizi tamamlayabilirsiniz.';
    else if (score >= 40) message = 'Farklılıklarınız zenginlik kaynağı. Öğrenme fırsatı çok!';
    else message = 'Zorlayıcı ama büyüme getiren bir bağ.';
