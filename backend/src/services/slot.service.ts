...
class SlotMachineManager {
    ...
    private state: 'BETTING' | 'ROLLING' | 'RESULT' = 'BETTING';
    private timeLeft: number = 26;
    private bets: Bet[] = [];
    private lastResult: { n1: number, n2: number, n3: number, sum: number } | null = null;
    ...
    private async tick() {
        if (this.state === 'BETTING' && this.timeLeft <= 0) {
            ...
            if (this.lastResult) await this.resolveBets(this.lastResult.sum);
    ...
    private async resolveBets(sum: number) {
    ...
    async placeBet(userId: string, betAmount: number, betType: 'BIG' | 'SMALL') {
        if (this.state !== 'BETTING') throw new Error('Şu an bahis alımları kapalı, zarlar dönüyor!');
        if (this.timeLeft <= 2) throw new Error('Bahis kapanmak üzere, lütfen sonraki turu bekleyin.');
...
export const slotManager = new SlotMachineManager();
